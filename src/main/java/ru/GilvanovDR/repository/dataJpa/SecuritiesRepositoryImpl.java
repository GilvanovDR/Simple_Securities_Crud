...
public class SecuritiesRepositoryImpl implements SecuritiesRepository {
    private static final Sort SORT_SEC_ID = Sort.by(Sort.Direction.ASC, "SEC_ID");
    ...
    @Transactional
    @Override
    public boolean saveAll(List<Security> securities) {
        return securitiesRepository.saveAll(securities).size()==securities.size();
    }
}
